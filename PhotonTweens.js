...
class PhotonTweens {
  ...
  constructor(start_pos, end_pos, infoText, color = 0x000000, duration = 6000) {
    this.firstTween;
    this.secondTween;
    this.thirdTween;

    this._createTweens(start_pos, end_pos, infoText, color, duration);
  }

  _createTweens(start_pos, end_pos, infoText, color, duration) {
    let x_dist, y_dist, x_sign, y_sign, photonSprite;

    photonSprite = this._createPhotonSprite(start_pos);
    ...
    this.firstTween = new TWEEN.Tween(photonSprite).to({
      x: x_sign + x_dist / 3,
      y: y_sign + y_dist / 3,
      alpha: 1,
    }, duration / 3);
    this.secondTween = new TWEEN.Tween(photonSprite).to({
      x: x_sign + x_dist / 3,
      y: y_sign + y_dist / 3,
    }, duration / 3);
    this.thirdTween = new TWEEN.Tween(photonSprite).to({
      x: x_sign + x_dist / 3,
      y: y_sign + y_dist / 3,
      alpha: 0,
    }, duration / 3);
  ...
  _createPhotonSprite(pos, color, rotation) {
    var photonSprite;
    var frames = [];
    ...
    photonSprite.scale.set(0.15);
    photonSprite.anchor.set(0, 0.5);
    photonSprite.position = pos;
    photonSprite.animationSpeed = 0.8;
    photonSprite.filters = [new PIXI.filters.ColorReplaceFilter(0x000000, color, 0.4)];
