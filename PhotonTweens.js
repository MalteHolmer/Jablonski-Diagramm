...
class PhotonTweens {
  ...
  constructor(start_pos, end_pos, infoText, replaceColor, duration) {
    this.firstTween;
    this.secondTween;
    this.thirdTween;
  ...
  _createTweens(start_pos, end_pos, infoText, replaceColor, duration) {
    ...
    /*Trigonometrische Überlegung, die Abhängig von der Ausrichtung des Photons
      berechnet, was in x und y abgezogen werden muss, um der Länge des Photons
      gerecht zu werden (travel-distanz verringert sich um den Vektor)*/
    let x_offset = Math.abs(Math.cos(angle)) * sprite_length;
    let y_offset = Math.abs(Math.sin(angle)) * sprite_length;

    //Fallunterscheidung, je nach Travel-richtung des Photons
    if (x_dist > 0) {
      x_dist -= x_offset;
    } else {
      x_dist += x_offset;
    ...
    //Erstellen der Tweens: Erscheinen -> Traveln -> Verschwinden
    ...
    //Ein InfoText wird gezeigt und die Animation gestartet, wenn die Tweens losgehen
  ...
  //erstellt ein neues Photon-Sprite, das mit einer bestimmten Farbe eingefärbt wird
  _createPhotonSprite(pos, replaceColor) {
    var photonSprite;
    var frames = [];

    //Spirte besteht aus 16 Texturen, die zu einem "AnimatedSprite" fusioniert werden
    for (var i = 1; i <= 16; i++) {
      frames.push(PIXI.Texture.fromFrame("photon-sprite_" + i + ".png"));
    }

    //AnimatedSprite wird eingerichtet und eingefärbt
    ...
    glb.tempGraphics.push(photonSprite);
    glb.app.stage.addChild(photonSprite);
