...
class AnimationControl {
  ...
  play() {
    if (this.playState == playState.IDLE && !this.isEmpty()) {
  ...
  _setChaining() {
    ...
    this.tweenChains.forEach(function (tweenChain, i, arr) {
      tweenChain.onComplete(function () {
        ...
        if (arr.length >= 2 && self.playState == playState.PLAY) {
          arr[1].start();
        } else if (self.playState == playState.STEP || self.playState == playState.PAUSE || arr.length == 1) {
          self.playState = playState.IDLE;
        }
        arr.shift();
      });
    });
  ...
  playDrag(rand_s1) {
    ...
    this.tweenChains[0] = new BarTweens(rand_s1, s1_bars.children);
